...
private:
    ...
    void _flush_block(block_id id) {
        uint8_t flag = this->_cache->_cache_flags[id];
        if (!(flag & this->_cache->_flag_using)) return;
        if (flag & this->_cache->_flag_dirty) {
            // write back to file
            fseek(this->_handle, this->_cache->_cache_offset[id], SEEK_SET);
            fwrite(this->_cache->_cache + id * this->_cache->_block_size, 1, this->_cache->_cache_blksize[id], this->_handle);
            // clear dirty flag
    ...
    void _force_load_data_to_block(block_id id, usize offset) {
        ...
        usize bytes_read = fread(this->_cache->_cache + id * this->_cache->_block_size, 1, this->_cache->_block_size, this->_handle);
        // set block info
    ...
    block_id _allocate_block() {
        ...
        memset(this->_cache->_cache + to_kick * this->_cache->_block_size, 0, this->_cache->_block_size);
    ...
    block_id _get_block_r(usize offset) {
        ...
        for (block_id i = 0; i < this->_cache->_block_count; i++) {
            if (!this->_is_using_block(i)) continue;
    ...
    usize _blk_read(usize offset, char *buffer, usize rel_offset, usize size) {
        ...
        memcpy(buffer, this->_cache->_cache + blk * this->_cache->_block_size + rel_offset, bytes_read);
        return bytes_read;
    }
    usize _blk_write(usize offset, char *buffer, usize rel_offset, usize size) {
        ...
        if (bytes_written > size) bytes_written = size;
        memcpy(this->_cache->_cache + blk * this->_cache->_block_size + rel_offset, buffer, bytes_written);
        // update block info
        usize new_blk_size = rel_offset + bytes_written;
        if (new_blk_size > blk_size) this->_cache->_cache_blksize[blk] = new_blk_size;
        return bytes_written;
    }
    // abstracted i/o operations
    usize _read(char *buffer, usize offset, usize size) {
        usize bytes_read = 0;
        usize blk_offset = (offset / this->_cache->_block_size) * this->_cache->_block_size;
        while (size > 0 && offset + bytes_read < this->_file_size) {
            usize rel_offset = offset >= blk_offset ? offset - blk_offset : 0;
            usize cur_read = this->_blk_read(blk_offset, buffer + bytes_read, rel_offset, size);
            size -= cur_read;
            bytes_read += cur_read;
    ...
    void _write(char *buffer, usize offset, usize size) {
        ...
        while (size > 0) {
            usize rel_offset = offset >= blk_offset ? offset - blk_offset : 0;
            usize cur_written = this->_blk_write(blk_offset, buffer + bytes_written, rel_offset, size);
            size -= cur_written;
            bytes_written += cur_written;
            blk_offset += this->_cache->_block_size;
        }
        usize new_file_size = offset + bytes_written;
        if (new_file_size > this->_file_size) this->_file_size = new_file_size;
        return;
    }
    void _flush() {
...
public:
    HybridFileHandle(HybridBlockCache *cache_impl, const char *path, const char *mode) {
...
inline bool isChar(uint8_t c) {
    return (c & 0x80) == 0;
} 

inline bool isNegative(uint32_t num) {
    return (num > 2e9);
}
...
private:
    ...
    size_t find_checkpoint_small(uint32_t pos) {
        ...
        while (l + 1 < r) {
            size_t mid = (l + r) / 2;
            if (checkpoints[mid].bwt_idx <= pos) l = mid;
            else r = mid;
        }
        return l;
    }
    ...
    void add_new_checkpoint(uint32_t *local_occ, uint32_t bwt_cnt, uint32_t rlb_cnt) {
